...
public class JSUnitRunnerIntegrationTest extends ServoyRunnerTestBase {
	...
	public void testActiveSolution_runAll_isMarkdownTable() throws Exception {
		...
		assertTrue("headless client run returned an error (" + result.substring(0, Math.min(result.length(), 80))
	...
	public void testActiveSolution_runAll_passCountIsParseable() throws Exception {
		...
		assertTrue("headless client run returned an error - pass-count assertion failed", !result.startsWith("Error"));
	...
	public void testActiveSolution_runNullScope_producesValidOutput() throws Exception {
		...
		assertTrue(
				...
				result.contains("| Passed") || result.contains("No "));
	}
	...
	public void testActiveSolution_runAll_failCountIsParseable() throws Exception {
		...
		assertTrue("headless client run returned an error - skipping fail-count assertion",
	...
	public void testActiveSolution_runAll_errorCountIsParseable() throws Exception {
		...
		assertTrue("headless client run returned an error - skipping error-count assertion",
				!result.startsWith("Error"));
	...
	public void testActiveSolution_runAll_totalCountIsPositive() throws Exception {
		...
		assertTrue("headless client run returned an error - skipping total-count assertion",
				!result.startsWith("Error"));
	...
	public void testActiveSolution_runAll_outputHasPassedOrFailureBranch() throws Exception {
		...
		assertTrue("headless client run returned an error - skipping branch-coverage assertion",
				!result.startsWith("Error"));
	...
	public void testActiveSolution_runAll_allPassedMessagePresent() throws Exception {
		...
		assertTrue("headless client run returned an error - skipping all-passed assertion",
				!result.startsWith("Error"));
	 ...
	 * {@code test_pilot_suite} if a different solution (e.g.
	 * {@code test_layer4_suite} left over by a preceding test class) is currently
	 * active. That matters for
	 * {@link #testActiveSolution_runAll_allPassedMessagePresent}, which requires a
	 * solution with zero errors.
	 * <ul>
	 * <li>Fast path: {@code test_pilot_suite} is already active - returns
	 * immediately.</li>
	 * <li>Otherwise: refreshes the model, locates {@code test_pilot_suite},
	 * activates it via a direct call on the SWT event thread, then pumps events
	 * until the model reports it as active.</li>
	 * <li>Falls back to the first available project if {@code test_pilot_suite} is
	 * not found (e.g. user-only workspace).</li>
	...
	private void ensureActiveServoyProject() throws Exception {
		...
		assertTrue("No ServoyProject found in the workspace after setup attempt - skipping Layer 3 tests",
		...
		if (toActivate == null)
			toActivate = projects[0]; // fallback: any project
		...
		// Pump SWT events so activation background jobs can deliver UI-thread
		// callbacks.
		// Thread.sleep() here holds the event thread and causes the timeout to always
		// fire.
		...
		assertNotNull("Active project not set after " + (ACTIVATE_SETTLE_MS / 1000)
				+ " seconds - Layer 3 tests cannot proceed", model.getActiveProject());
	}

	/**
	 * Returns true if {@code test_pilot_suite} is the currently active Servoy
	 * project.
	 */
