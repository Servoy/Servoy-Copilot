...
public class McpServer
{
	...
	public McpServer(Map<ToolSpecification, ToolExecutor> tools)
	{
		this(tools, defaultServerInfo());
	}

	public McpServer(Map<ToolSpecification, ToolExecutor> tools, McpImplementation serverInfo)
	{
		...
		Map<String, ToolExecutor> executors = tools.entrySet().stream()
			.collect(Collectors.toMap(
				entry -> entry.getKey().name(), // New Key
				entry -> entry.getValue() // Same Value
			));
	...
	private McpInitializeResult handleInitialize(McpInitializeRequest request)
	{
		...
		if (request.getParams() != null && (request.getParams() instanceof McpInitializeParams mcpInitializeParams) &&
