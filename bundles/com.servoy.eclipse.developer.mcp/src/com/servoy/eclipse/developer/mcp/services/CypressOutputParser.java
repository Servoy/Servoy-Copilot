...
public final class CypressOutputParser {
	...
	public static TestStatus determineStatus(String rawOutput) {
		...
		if (rawOutput.contains("Some tests failed")) {
			// Distinguish crashes/uncaught errors from assertion failures
	...
	public static String extractErrorSummary(String rawOutput, TestStatus status) {
		...
		if (status == TestStatus.ERROR) {
			// Try to find a meaningful error line (skip warnings and blank lines)
		...
		// FAILED status - find the actual assertion error line. Cypress prints failures like:
		//   1) suite name
		//        test name:
		//      AssertionError: expected ... / CypressError: ... / Timed out retrying ...
		// Skip Cypress box-drawing separators and boilerplate.
	...
	/** Returns true if the line is only separator/box-drawing characters. */
	...
	/**
	 * Extracts the path to the Cypress video (.mp4) referenced in the output, or null if
	 * none is present or the file no longer exists. Paths have already been rewritten to
	 * the preserved-artifacts location by FormSpecRunner.preserveArtifacts().
	 */
	...
	/**
	 * Extracts the path to the first Cypress screenshot (.png) referenced in the output, or
	 * null if none is present or the file no longer exists.
	 */
	...
	/**
	 * Reads a preserved-media path from the marker line that FormSpecRunner appends
	 * ("[Preserved Video] &lt;path&gt;" / "[Preserved Screenshot] &lt;path&gt;"). Returns null
	 * if the marker is absent or the file no longer exists.
	 */
