...
public class ServoyTestingServer
{
		...
		try (java.util.stream.Stream<java.nio.file.Path> files = java.nio.file.Files.walk(supportDir))
		{
			...
			files.filter(p -> {
				...
				try
				{
					...
					// Skip files that only have scaffold comments (no real commands)
	...
	@Tool(name = "runJsUnitTests", description = "Runs JSUnit tests for the active Servoy solution and returns pass/fail results with failure traces. " +
		"Use this to verify tests pass after creating or modifying test files, or to identify which tests are currently failing. " +
		"Returns a markdown summary with counts and detailed failure/error traces.", type = "object")
	public String runJsUnitTests(
		@ToolParam(name = "scopeOrAll", description = "What to test: a scope/file name (e.g. 'test_utils' or 'test_utils.js'), a form name (e.g. 'tab1' or 'forms/tab1.js'), a module name (e.g. 'calculations_module'), 'MODULES' to run all tests across every module of the active solution, or 'ALL' to run every test in the active solution itself (its own scopes and forms only, NOT its modules)") String scopeOrAll,
		@ToolParam(name = "timeoutSeconds", description = "Maximum seconds to wait for the test run to complete. Use 60 for a single scope or form, 120 for a full solution run.", type = "integer") int timeoutSeconds)
	{
		try
		{
	...
	@Tool(name = "runTestMethod", description = "Runs a single named JSUnit test method in the active Servoy solution and returns its result. " +
		"Executes the full test scope containing the method (narrowed by scopeOrAll if provided) " + "and filters results to the specified method name. " +
		"Use runJsUnitTests to run all tests in a scope.", type = "object")
	public String runTestMethod(
		@ToolParam(name = "testMethodName", description = "Name of the test function to run, e.g. 'testCalculateTax'. Case-insensitive.") String testMethodName,
		@ToolParam(name = "scopeOrAll", description = "Scope to search: a form name, module name, 'ALL', or null to run the whole solution.", required = false) String scopeOrAll,
		@ToolParam(name = "timeoutSeconds", description = "Timeout in seconds to wait for the test run to complete.", type = "integer") int timeoutSeconds)
	{
		try
		{
	...
	public String getJSUnitCoverageReport(
		@ToolParam(name = "coveragePath", description = "Absolute path to the coverage JSON file. If omitted, uses ${workspace}/jsunit-coverage.json.", required = false) String coveragePath)
	{
		try
		{
	...
	public String suggestTestsFromCoverage(
		@ToolParam(name = "coveragePath", description = "Absolute path to the coverage JSON file. If omitted, uses ${workspace}/jsunit-coverage.json.", required = false) String coveragePath,
		@ToolParam(name = "maxFunctions", description = "Maximum number of functions to report on, sorted by uncovered line count descending. Default: 20.", type = "integer", required = false) int maxFunctions)
	{
		try
		{
	...
	@Tool(name = "checkNGClientStatus", description = "Checks whether the NG client is currently running and returns the base URL if available. " +
		"Use this before running form tests to verify the runtime is ready.", type = "object")
	...
	@Tool(name = "testForm", description = "Runs a single-form Cypress spec test in a headless browser. " +
		"The form must have a .spec.cy.js file in jenkins-custom/e2e-test-scripts/cypress/cy-form/ " +
		"(auto-generated by generateFormSpec or showFormInBrowser if missing). " +
		"This is for FORM TESTS (single form), not E2E tests (use testE2E for multi-form E2E tests). " +
		"Returns pass/fail results for each assertion in the spec. " + "Requires the NG client to be running (use checkNGClientStatus first).", type = "object")
	public String testForm(
		@ToolParam(name = "formName", description = "The name of the form to test (e.g. 'mainForm', 'orderDetails')") String formName)
	{
		try
		{
			ensureTestingMode();
	...
	@Tool(name = "showAndTest", description = "Opens a Servoy form in a visible browser AND runs Cypress spec tests against it. " +
		"Combines showFormInBrowser + testForm: the form is shown in a real browser window " + "and assertions from the .spec.cy.js file are executed. " +
		"Auto-generates the spec file if it doesn't exist. " + "Returns pass/fail results.", type = "object")
	public String showAndTest(
		@ToolParam(name = "formName", description = "The name of the form to show and test (e.g. 'mainForm', 'orderDetails')") String formName)
	{
		try
		{
			ensureTestingMode();
	...
	@Tool(name = "listE2ETests", description = "Lists all Cypress E2E test files (.cy.js) in jenkins-custom/e2e-test-scripts/cypress/e2e/. " +
		"These are multi-form navigation/E2E tests, NOT single-form tests (form tests are in cypress/cy-form/). " +
		"Use this to discover which E2E tests exist before calling testE2E or showAndTestE2E. " +
		"Returns file names, the form name each file targets, and the full path.", type = "object")
	public String listE2ETests()
	{
		try
		{
			...
			StringBuilder sb = new StringBuilder();
			sb.append("**E2E Tests** (").append(specs.size()).append(" file").append(specs.size() == 1 ? "" : "s")
				.append(")\n\n");
			...
			sb.append("\nPass the form name (e.g. '")
				.append(specs.get(0).getFileName().toString().replaceAll("\\.cy\\.(js|ts)$", ""))
	...
	@Tool(name = "testE2E", description = "Runs Cypress E2E tests for a Servoy form in headless mode. " +
		"Looks for a .cy.js file in jenkins-custom/e2e-test-scripts/cypress/e2e/<solutionName>/ (generated by generateCypressE2ETest). " +
		"Uses the cypress.config.js in jenkins-custom/e2e-test-scripts/. " + "Returns pass/fail results for each assertion. " +
		"Requires the NG client to be running (use checkNGClientStatus first).", type = "object")
	public String testE2E(
		@ToolParam(name = "targetForm", description = "The form name to test (e.g. 'order_detail'). Looks for <formName>.cy.js in the E2E directory.") String targetForm)
	{
		try
		{
			ensureTestingMode();
	...
	@Tool(name = "showAndTestE2E", description = "Runs Cypress E2E tests for a Servoy form in headed (visible browser) mode. " +
		"Looks for a .cy.js file in jenkins-custom/e2e-test-scripts/cypress/e2e/<solutionName>/ (generated by generateCypressE2ETest). " +
		"Uses the cypress.config.js in jenkins-custom/e2e-test-scripts/. " + "The browser window is visible so you can watch the test execute. " +
		"Returns pass/fail results. " + "Requires the NG client to be running (use checkNGClientStatus first).", type = "object")
	...
	@Tool(name = "generateFormSpec", description = "Creates a FORM TEST for a single Servoy form. " +
		"USE THIS when the user asks to 'create form test', 'generate form test', or 'add form test'. " +
		"This is NOT for E2E tests (use generateCypressE2ETest for multi-form navigation/E2E tests). " +
		"Creates two files: a .spec.cy.js in jenkins-custom/e2e-test-scripts/cypress/cy-form/ (Cypress UI assertions with data-cy selectors) " +
		"and a .spec.js in jenkins-custom/e2e-test-scripts/cypress/cy-form-spec/ (Servoy setUp/tearDown scope with DLTK code completion). " +
		"Reads the form's .frm file to extract element names, dataSource, and structure. " +
		"Requires servoy.ngclient.testingMode=true in servoy.properties for data-cy attributes.", type = "object")
	public String generateFormSpec(
		@ToolParam(name = "formName", description = "The name of the form to generate a spec for (e.g. 'mainForm', 'orderDetails')") String formName)
	{
		try
		{
	...
	@Tool(name = "executeTestSetup", description = "Inserts a test record into a database table via JDBC. " +
		"Use this BEFORE running Cypress tests to ensure the form has data to display. " +
		"The record is inserted directly into the database, bypassing Servoy's foundset layer. " + "Call executeTestTeardown after testing to clean up. " +
		"Returns success message with row count or error details.", type = "object")
	public String executeTestSetup(
		@ToolParam(name = "serverName", description = "Database server name (e.g. 'example_data')") String serverName,
		@ToolParam(name = "tableName", description = "Table name (e.g. 'orders')") String tableName,
		@ToolParam(name = "columnValuesJson", description = "Column values as JSON string (e.g. '{\"customerid\":\"CYPRS\",\"shipcity\":\"TestCity\"}')") String columnValuesJson)
	{
	...
	@Tool(name = "executeTestTeardown", description = "Deletes test records from a database table via JDBC. " +
		"Use this AFTER running Cypress tests to clean up test data created by executeTestSetup. " + "Deletes all rows matching the given where clause. " +
		"Returns success message with deleted row count or error details.", type = "object")
	public String executeTestTeardown(
		@ToolParam(name = "serverName", description = "Database server name (e.g. 'example_data')") String serverName,
		@ToolParam(name = "tableName", description = "Table name (e.g. 'orders')") String tableName,
		@ToolParam(name = "whereColumn", description = "Column name for the WHERE clause (e.g. 'shipcity')") String whereColumn,
		@ToolParam(name = "whereValue", description = "Value to match in the WHERE clause (e.g. 'CYPRESS_TEST_CITY')") String whereValue)
	{
		...
		@ToolParam(name = "solutionName", description = "Solution name to create the file in. Use 'TARGET' to use the current active solution.") String solutionName)
	{
		try
		{
			...
			{
				com.servoy.eclipse.model.nature.ServoyProject activeProject = com.servoy.eclipse.core.ServoyModelManager
			...
			return com.servoy.eclipse.developer.mcp.services.TestFileService.getInstance().createTestFile(testFileName,
				actualSolutionName);
		}
		catch (Exception e)
		{
	...
	@Tool(name = "addTestMethod", description = "Adds or replaces a test method in an existing JSUnit test file. " +
		"If a method with the same name already exists it is replaced (duplicates are also removed). " +
		"Generates proper @properties annotation with UUID automatically.", type = "object")
	public String addTestMethod(@ToolParam(name = "testFileName", description = "Test file name (e.g., 'test_utils.js')") String testFileName,
		@ToolParam(name = "testMethodName", description = "Test method name (must start with 'test_', e.g., 'test_calculateTotal_withDiscount')") String testMethodName,
		@ToolParam(name = "testCode", description = "Complete test function body (without function declaration or @properties)") String testCode)
	{
		try
		{
			com.servoy.eclipse.model.nature.ServoyProject activeProject = com.servoy.eclipse.core.ServoyModelManager
			...
			return com.servoy.eclipse.developer.mcp.services.TestFileService.getInstance().addTestMethod(testFileName,
				testMethodName, testCode, solutionName);
		}
		catch (Exception e)
		{
	...
	@Tool(name = "generateTestCases", description = "Suggests test cases for a function based on its signature and logic. " +
		"Returns structured list of test scenarios covering happy path, edge cases, and error conditions. " +
		"Does NOT create files - only provides suggestions for test coverage.", type = "object")
	public String generateTestCases(
		@ToolParam(name = "sourceCode", description = "Source code or function signature to analyze") String sourceCode,
		@ToolParam(name = "functionName", description = "Function name to generate test cases for") String functionName)
	{
		try
		{
	...
	@Tool(name = "analyzeCodeForTesting", description = "Analyzes selected code to identify testable functions. " +
		"Returns function name and parameters extracted from the code snippet.", type = "object")
	public String analyzeCodeForTesting(
		@ToolParam(name = "selection", description = "Selected code snippet to analyze for testable functions") String selection)
	{
		try
		{
	...
	private static String extractFunctionName(String code)
	{
		if (code == null)
			return null;
		java.util.regex.Matcher matcher = java.util.regex.Pattern.compile("function\\s+(\\w+)\\s*\\(").matcher(code);
		return matcher.find() ? matcher.group(1) : null;
	}

	private static String[] extractTestParameters(String code, String functionName)
	{
		if (code == null || functionName == null)
			return new String[0];
		java.util.regex.Matcher matcher = java.util.regex.Pattern
			.compile("function\\s+" + java.util.regex.Pattern.quote(functionName) + "\\s*\\(([^)]*)").matcher(code);
		if (matcher.find())
		{
			String paramStr = matcher.group(1).trim();
			if (paramStr.isEmpty())
				return new String[0];
			return java.util.Arrays.stream(paramStr.split(",")).map(String::trim).filter(s -> !s.isEmpty())
				.toArray(String[]::new);
		}
		return new String[0];
	}
	...
	@Tool(name = "findForm", description = "Finds Servoy forms by natural-language description, UI title, internal name, or datasource. " +
		...
		"Pass the returned internal name to other tools like getFormNavigationGraph, showFormInBrowser, or getSource.", type = "object")
	public String findForm(
		@ToolParam(name = "query", description = "The description, UI title, or partial name to search for (e.g. 'Query performance popup', 'customer', 'orders'). Leave blank to list all forms.", required = false) String query)
	{
		try
		{
	...
	@Tool(name = "getFormNavigationGraph", description = "Returns the form navigation graph of the active Servoy solution as JSON. " +
		"This is for E2E test planning only — do NOT use for single-form tests (use generateFormSpec for form tests). " +
	...
	public String getFormNavigationGraph(
		@ToolParam(name = "formName", description = "Form name to filter the graph to only edges relevant for reaching this form. If omitted, returns the full navigation graph.", required = false) String formName,
		@ToolParam(name = "summaryOnly", description = "If true, returns a compact summary (parent->children map) instead of full edge objects. Use for large solutions to avoid token limits. Default: false.", type = "boolean", required = false) boolean summaryOnly)
	{
		try
		{
			...
			if (summaryOnly)
			{
				...
				{
					parentToChildren.computeIfAbsent(edge.getFrom(), k -> new java.util.ArrayList<>())
						.add(edge.getTo());
				}
				...
				if (formName != null && !formName.trim().isEmpty())
				{
					...
					for (NavigationEdge step : path)
					{
						...
						if (step.getCypressSelector() != null)
							stepNode.put("cypressSelector", step.getCypressSelector());
			...
			// full mode: return compact JSON (not pretty-printed) to stay within token
			// limits
	...
	public String getNavigationPath(
		@ToolParam(name = "targetForm", description = "The target form name to navigate to (e.g. 'order_detail')") String targetForm,
		@ToolParam(name = "fromForm", description = "Starting form name. If omitted, starts from the solution's first/main form.", required = false) String fromForm)
	{
		try
		{
	...
	private com.fasterxml.jackson.databind.node.ObjectNode edgeToJson(
		com.fasterxml.jackson.databind.ObjectMapper mapper, NavigationEdge edge)
	{
		...
		if (edge.getPropertyName() != null)
			node.put("propertyName", edge.getPropertyName());
		if (edge.getTabName() != null)
			node.put("tabName", edge.getTabName());
		if (edge.getTabIndex() >= 0)
			node.put("tabIndex", edge.getTabIndex());
		if (edge.getRelationName() != null)
			node.put("relationName", edge.getRelationName());
		if (edge.getTrigger() != null)
			node.put("trigger", edge.getTrigger());
	...
	public String generateCypressE2ETest(
		@ToolParam(name = "targetForm", description = "The target form to reach and test (e.g. 'dialogform1').") String targetForm,
		@ToolParam(name = "scenario", description = "Plain-English description of what the test should verify. Embedded as a comment in the generated file.") String scenario,
		@ToolParam(name = "fromForm", description = "Form to start navigation from. Defaults to the solution main form.", required = false) String fromForm,
		@ToolParam(name = "outputFileName", description = "Output file name (e.g. 'dialog_flow.cy.ts'). Defaults to '<targetForm>.cy.ts'.", required = false) String outputFileName,
		@ToolParam(name = "baseUrl", description = "Base URL of the running Servoy app (e.g. 'http://localhost:8183'). Defaults to the server's actual port.", required = false) String baseUrl,
		@ToolParam(name = "loginUrl", description = "URL where the login form is located. Required when the solution needs authentication.", required = false) String loginUrl,
		@ToolParam(name = "testUsername", description = "Test user username for login. Required when the solution needs authentication.", required = false) String testUsername,
		@ToolParam(name = "testPassword", description = "Test user password for login. Required when the solution needs authentication. Never echoed in output.", required = false) String testPassword,
		@ToolParam(name = "loginSuccessSelector", description = "CSS selector that proves login succeeded (e.g. '#main-menu'). Required when the solution needs authentication.", required = false) String loginSuccessSelector)
	{
		try
		{
			...
			boolean hasLoginCreds = testUsername != null && !testUsername.isBlank() && testPassword != null && !testPassword.isBlank();
			...
			java.nio.file.Path workspaceRoot = java.nio.file.Paths.get(
				org.eclipse.core.resources.ResourcesPlugin.getWorkspace().getRoot().getLocation().toOSString());
			java.nio.file.Path e2eDir = workspaceRoot.resolve("jenkins-custom").resolve("e2e-test-scripts")
				.resolve("cypress").resolve("e2e").resolve(solutionName);
			...
			// Discover existing custom commands/helpers to include as context for the AI
			StringBuilder result = new StringBuilder();
			result.append("Created: ").append(relativePath).append("\n\n").append(content);

			String helpers = discoverCypressHelpers(supportDir);
			if (helpers != null && !helpers.isBlank())
			{
				result.append("\n\n--- Available Cypress custom commands (from cypress/support/) ---\n");
				result.append("Use these in the generated test instead of writing raw commands:\n\n");
