...
public class ServoyTestingServer {
	...
	@Tool(name = "runTestMethod", description = "Runs a single named JSUnit test method in the active Servoy solution and returns its result. "
			+ "Executes the full test scope containing the method (narrowed by scopeOrAll if provided) "
			+ "and filters results to the specified method name. "
			+ "Use runJsUnitTests to run all tests in a scope.", type = "object")
	public String runTestMethod(
			@ToolParam(description = "Name of the test function to run, e.g. 'testCalculateTax'. Case-insensitive.") String testMethodName,
			@ToolParam(description = "Scope to search: a form name, module name, 'ALL', or null to run the whole solution.") String scopeOrAll,
			@ToolParam(description = "Timeout in seconds to wait for the test run to complete.", type = "integer") int timeoutSeconds) {
		try {
	...
	@Tool(name = "showFormInBrowser", description = "Opens a specific Servoy form in an external browser for preview/testing. "
			...
			+ "Returns the URL that was opened.", type = "object")
	...
	public String listE2ETests() {
		try {
			...
			try (java.util.stream.Stream<java.nio.file.Path> stream = java.nio.file.Files.walk(e2eDir)) {
				specs = stream
						.filter(p -> p.getFileName().toString().endsWith(".cy.js")
								|| p.getFileName().toString().endsWith(".cy.ts"))
	...
	@Tool(name = "showAndTestE2E", description = "Runs Cypress E2E tests for a Servoy form in headed (visible browser) mode. "
			+ "Looks for a .cy.js file in jenkins-custom/e2e-test-scripts/cypress/e2e/<solutionName>/ (generated by generateCypressE2ETest). "
			+ "Uses the cypress.config.js in jenkins-custom/e2e-test-scripts/. "
			+ "The browser window is visible so you can watch the test execute. " + "Returns pass/fail results. "
			+ "Requires the NG client to be running (use checkNGClientStatus first).", type = "object")
	...
	public String addTestMethod(@ToolParam(description = "Test file name (e.g., 'test_utils.js')") String testFileName,
