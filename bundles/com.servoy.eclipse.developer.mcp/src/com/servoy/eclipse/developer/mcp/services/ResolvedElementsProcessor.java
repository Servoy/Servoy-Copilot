...
/**
 * Processes {@link SelectionResult} (model + foreign elements) into structured JSON.
 * <p>
 * Ported from {@code com.servoy.eclipse.servoypilot.tools.codecontext.ResolvedElementsProcessor}.
 * Includes Emera's enhancements from SVY-21047: IField support and richer persist context
 * (Form, ValueList, Relation, WebComponent, FormElement).
 * </p>
 */
...
public class ResolvedElementsProcessor
{
	...
	public void processElementSource(ObjectNode node, Element elementSource)
	{
		...
		if (resource instanceof IFile file)
		{
			...
			hints.add("Use readProjectResource tool to read this file");
		}
	}
	...
	private String extractRelationContext(ObjectNode node, Relation relation)
	{
		...
		ArrayNode hints = getOrCreateHints(node);
		hints.add("Use readProjectResource tool to read the .rel file and .dbi files to check record properties");
	...
	private static String readWorkspaceFile(String filePath)
	{
		...
		{
			ServoyLog.logError("ResolvedElementsProcessor: error reading file: " + filePath, e);
		}
		return "";
	}
}
