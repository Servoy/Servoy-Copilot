...
/**
 * MCP server providing workspace context tools for the Servoy Developer MCP
 * endpoint.
 * <p>
 ...
 * Tools:
 * <ul>
 * <li>{@code listCachedResources} - lists all resources in the Servoy
 * resource cache</li>
 * <li>{@code getCachedResource} - returns content of a specific cached
 * resource by URI</li>
 * <li>{@code getCacheStats} - cache statistics</li>
 * <li>{@code getFileHistory} - lists Eclipse Local History versions of a
 * file</li>
 * <li>{@code getFileHistoryContent} - returns content of a specific history
 * version</li>
 * <li>{@code compareWithHistory} - unified diff between current file and a
 * history version</li>
 * </ul>
 * </p>
 */
...
public class ServoyContextServer {
	...
	@Tool(name = "listCachedResources", description = "Lists all resources currently cached in the Servoy Developer MCP workspace context. "
			...
			+ "Use this to see what files the AI agent has recently accessed.", type = "object")
	public String listCachedResources() {
		...
		for (CachedEntry e : all.values()) {
			...
			sb.append(String.format("%-50s  %-20s  v%-4d  %-20s  ~%d\n", uri, e.type(), e.version(),
					TIMESTAMP_FMT.format(e.cachedAt()), e.estimateTokens()));
		}

		return sb.toString();
	}

	@Tool(name = "getCachedResource", description = "Gets the content of a specific cached resource by URI without re-reading from disk. "
			+ "Use listCachedResources first to see available URIs. "
			+ "Returns the cached version - fast, no I/O.", type = "object")
	public String getCachedResource(
			@ToolParam(name = "resourceUri", description = "The URI of the cached resource (e.g. 'workspace:///ProjectName/scopes/utils.js')", required = true) String resourceUri) {
		return cache.get(resourceUri).map(e -> {
			StringBuilder sb = new StringBuilder();
			sb.append("# ").append(e.displayName()).append(" (v").append(e.version()).append(", cached ")
					.append(TIMESTAMP_FMT.format(e.cachedAt())).append(")\n\n");
			sb.append(e.content());
			return sb.toString();
		}).orElse("Resource not found in cache: " + resourceUri + "\nUse listCachedResources to see available URIs.");
	}

	@Tool(name = "getCacheStats", description = "Gets resource cache statistics: number of resources, token usage, and limits.", type = "object")
	...
	@Tool(name = "getFileHistory", description = "Lists the Local History versions of a file maintained by Eclipse. "
			+ "Shows timestamps and sizes for each historical version. "
			+ "Eclipse automatically saves file history on every modification through the IDE.", type = "object")
	...
	@Tool(name = "getFileHistoryContent", description = "Gets the content of a specific Local History version of a file. "
			+ "Use getFileHistory first to see available versions and their indices.", type = "object")
	...
	@Tool(name = "compareWithHistory", description = "Shows a unified diff between the current file content and a Local History version. "
			+ "Use getFileHistory to find the version index.", type = "object")
